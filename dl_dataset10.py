...
#pore forming proteins - dl dataset, equal samples from all classes', only physico chemical

import pandas as pd 
import numpy as np
from Bio import SeqIO
from sklearn.metrics import classification_report, accuracy_score, f1_score, roc_auc_score, matthews_corrcoef
...
def load_and_merge_data(fasta_files, pseaac_paths, physiochem_paths, esm_paths,prot_paths, labels, train_ids, test_ids):
    ...
    physio_df = pd.read_csv(config['physico_csv'])
    full_df = pd.concat(combined_df_list, axis=0, ignore_index=True)
    full_df = pd.merge(full_df, physio_df, on='sequence_id', how='inner')
    full_df = full_df.drop_duplicates(subset='sequence_id', keep='first')
...
train_df, test_df = load_and_merge_data(all_fasta_files, all_pseaac_paths, all_physiochem_paths, all_esm_embeddings_paths, all_protbert_paths, all_labels, train_ids=train_ids, test_ids=test_ids)
print(train_df.head())
