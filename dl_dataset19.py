...
negative_esm = os.path.join(config['embedding_dir'], "Cullpdb_5616HitsRemoved_embeddings.csv")


# Combine all positive embeddings
positive_esm_dfs = [pd.read_csv(path) for path in positive_esm]
positive_esm_df = pd.concat(positive_esm_dfs)

negative_esm_df = pd.read_csv(negative_esm)

esm_df = pd.concat([positive_esm_df, negative_esm_df])

df = df.merge(esm_df, on='sequence_id', how='left')
...
digit_columns = [col for col in feature_data.columns if re.fullmatch(r'\d+', col)]


train_data = feature_data[feature_data["is_Train"] == 1][digit_columns + ["label"]]
test_data = feature_data[feature_data["is_Train"] == 0][digit_columns + ["label"]]
...
with open("check_col.txt", "w") as f:
